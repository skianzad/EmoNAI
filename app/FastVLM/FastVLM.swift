...
private enum Vision {
    ...
    fileprivate class VisionModelCoreML {
        ...
        public func encode(_ image: MLXArray) -> MLXArray {
            ...
            precondition(
                image.dim(2) == 1024 && image.dim(3) == 1024,
                "Vision encoder expects a 1024×1024 input image, "
                    + "but received \(image.dim(2))×\(image.dim(3)). "
                    + "Check that the image preprocessing pipeline is normalising "
